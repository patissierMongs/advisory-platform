...
def build() -> Path:
    ...
    readme.write_text(
        ...
        "   자산관리대장 탭에서 가져오세요(자동 헤더 감지로 충분 — 수동이면 시작행 3·행수 2).\n"
        ...
        "5) 게시판까지 한 번에: 서버 실행 상태에서\n"
        "     python scripts/run_board_demo.py\n"
        "   → 위 1~4 전체 + 게시판 게시 + 부서 회신 댓글(조치완료·증빙 첨부/진행중/조치불가/\n"
        "   일반 질의)이 자동 구성되어 /board 를 바로 테스트할 수 있습니다.\n",
        encoding="utf-8")
    return OUT
