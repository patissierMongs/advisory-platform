...
class ScanRun(TimestampMixin, Base):
    """포트스캔 회차 — 독립 스캐너(portscan-tool)의 nmap XML 회차를 콘솔로 import."""
    __tablename__ = "scan_run"
    label: Mapped[str] = mapped_column(String(120), nullable=False)
    network: Mapped[str | None] = mapped_column(String(20))     # biz/inet/기타(자유)
    source_name: Mapped[str | None] = mapped_column(String(200))  # 업로드 zip 파일명
    ...
    identified_count: Mapped[int] = mapped_column(Integer, default=0, nullable=False)

    ports: Mapped[list["ScanPort"]] = relationship(
        back_populates="run", cascade="all, delete-orphan")


class ScanPort(TimestampMixin, Base):
    """회차 내 (호스트, 포트) 1건 — 상태 + 식별(서비스/버전/근거)."""
    __tablename__ = "scan_port"
    scan_run_id: Mapped[int] = mapped_column(
        ForeignKey("scan_run.id"), nullable=False, index=True)
    ...
    extra: Mapped[str | None] = mapped_column(String(200))
    identified: Mapped[str] = mapped_column(String(2), default="N", nullable=False)  # Y/N/P
    evidence: Mapped[str | None] = mapped_column(Text)
    banner: Mapped[str | None] = mapped_column(Text)
    note: Mapped[str | None] = mapped_column(String(200))

    run: Mapped["ScanRun"] = relationship(back_populates="ports")
