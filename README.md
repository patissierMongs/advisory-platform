...
> **올인원 데모 샘플**: `samples/demo_advisories/` 에 폴더 구조 그대로 업로드 존에 드래그하면
> 출처 자동 탐지 4단계(상위 폴더명→폴더명→파일명→본문)와 미탐지 `'-'` 케이스, CVE 게이트
> 직접 등록·색상 하이라이트를 전부 시연할 수 있는 권고문 PDF 8건이 들어 있습니다.
> 동봉된 `cve_feed_demo.json` 은 이 PDF 들의 **CVE 코드 12건 전체를 커버**하므로
> CVE 데이터베이스 탭에서 반입·적용하면 게이트가 일괄 해제됩니다(적용 순서는 폴더 안
> `READ ME` 참고). 재생성: `python scripts/make_demo_samples.py [--zip 경로]`.
>
> - `samples/cve_feed_extended_2026-07.json` — 위 12건을 **포함**하는 확장 피드(30건).
>   영향버전 규칙 전 형태(열거 / lt·lte / range / `*`)를 고루 담아 매칭·버전 비교기까지 시연.
> - `samples/자산관리대장_엣지케이스_샘플.xlsx` — 일부러 복잡한 현장 취합본을 재현한
>   자산대장: 병합 제목행·2단 병합 헤더·부서 세로 병합·제품+버전 한 셀·v접두/'알 수 없음'
>   버전·파일 내 중복 자산번호·부서/제품/버전 누락·숫자 자산번호·담당자 통합 셀.
>   자동 헤더 감지·추천 매핑만으로 21행 적재 + 경고 8건이 기대대로 나오는지 확인하는 용도.
> - **게시판까지 한 번에**: 서버 실행 상태에서 `python scripts/run_board_demo.py` —
>   업로드→피드→자산대장→매칭→게시판 게시→부서 회신 댓글(조치완료+증빙/진행중/조치불가/질의)
>   전체를 자동 구성해 `/board` 를 즉시 테스트할 수 있습니다.
...
- **`/admin`** — 기존 SPA. 관제 인원이 권고문 업로드·CVE 추출·자산 매칭·발송·조치추적을 수행.
  게시판은 사이드바가 아닌 **헤더 우측 상단 "게시판" 바로가기**로 연결(모든 이동은 현재 창 — 새 탭 없음). CVE 피드 갱신도 **드래그&드롭** 지원.
  권고문 카드 하단에 **처리 단계 색상 막대**(접수→CVE 추출→DB 조회→자산 매칭→발송·완료), 발송 이력 탭은 **5초 주기 자동 갱신**.
  증빙은 **다중 파일**(댓글·관리자 추가 업로드 모두)이며 파일명 클릭으로 팝업 열람 — 담당자 댓글 여러 건의 첨부가 발송이력에 **전부 누적**됩니다.
